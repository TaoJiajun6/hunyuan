...
**2. 精巧的对话设计**
   ...
   - **情绪表达（重要）**：角色之间要有丰富的情绪呼应和共鸣，体现真实的交流感
   - **情绪变化**：每个角色在不同对话中要有情绪变化，体现真实的情感波动
   - **情绪与内容匹配**：情绪标注必须与对话内容高度匹配，不能生硬添加
...
**5. 剧本格式**（SoulX-Podcast兼容格式）：
   - **强烈推荐使用情绪标注格式**：`[角色名]（情绪地）对话内容`
   - 基础格式（不推荐，缺少情绪）：`[角色名]对话内容`
   - **重要**：不要使用冒号，直接写对话内容
   - **情绪表达要求（重要）**：每段对话必须包含合适的情绪标注，让对话充满情感和生命力
   - 情绪标注示例：
     * 积极情绪：兴奋地、热情地、开心地、激动地、惊喜地、赞叹地、赞同地
     * 中性情绪：冷静地、平静地、思考状、认真地、专注地、客观地
     * 消极情绪：疑惑地、担忧地、严肃地、质疑地、不满地、失望地
     * 互动情绪：开玩笑地、调侃地、反驳地、接话、打断、恍然大悟地
     * 其他：惊讶地、恍然大悟地、若有所思地、点头、摇头、总结性地
   - **情绪变化**：同一角色在不同对话中要有情绪变化，体现真实的情感波动
   - **情绪呼应**：角色之间要有情绪上的呼应，如一方兴奋时另一方可以惊讶或赞同
   - 每行一个角色的发言，角色之间建议有空行间隔，让对话更清晰
   - 角色名称必须使用：{role_list}
   - **每段对话必须包含情绪标注**，让对话充满情感色彩
...
- **每段对话必须包含情绪标注**，让对话充满情感色彩
- 播客时长约5-7分钟
- 对话总字数建议3500-4500字（纯对话内容，确保内容充实）
...
**带情绪标注格式（强烈推荐）**：
```
[角色A]（热情地）嘿，听众朋友们，欢迎回到我们的频道！今天咱们可有个大话题要聊。

[角色B]（兴奋地）没错，是关于AI能否真正理解人类的幽默。你说，它能听懂咱们的梗吗？

[角色A]（开玩笑地）好家伙，上来就挑战高难度！我觉得吧，它现在可能还在学习为什么"香蕉滑倒了"是个笑话。

[角色B]（思考状）嗯...这个问题确实很有意思。从技术角度看，AI理解幽默的关键在于...

[角色A]（恍然大悟地）哦！我明白了，你是说它需要理解上下文和背景知识？

[角色B]（赞同地）对！而且还要能识别反讽、双关这些语言技巧。
```

**注意**：每段对话都应该有情绪标注，让对话充满情感和生命力！
...
- **剧本格式**（SoulX-Podcast兼容格式）：
   ...
   - 基础格式（不推荐，缺少情绪）：`[角色名]对话内容`
   - **重要**：不要使用冒号，直接写对话内容
   - **情绪表达要求**：每段对话必须包含合适的情绪标注，让对话充满情感和生命力
   - 情绪标注示例：
     * 积极情绪：兴奋地、热情地、开心地、激动地、惊喜地、赞叹地、赞同地
     * 中性情绪：冷静地、平静地、思考状、认真地、专注地、客观地
     * 消极情绪：疑惑地、担忧地、严肃地、质疑地、不满地、失望地
...
**示例：带情绪标注格式（强烈推荐）- {num_characters}个角色**
```
{self._generate_example_format(role_names, num_characters, with_emotion=True)}
```

**注意**：每段对话都应该有情绪标注，让对话充满情感和生命力！情绪要丰富多样，体现真实的情感波动。
