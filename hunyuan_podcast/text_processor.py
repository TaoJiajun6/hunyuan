...
- 整体风格要与分类特点保持一致"""
        ...
        # 构建完整的提示词（优化后的清晰结构）
        prompt = f"""【核心任务】
你是一位专业的播客编剧。请将提供的文本素材转化为一段多角色播客对话脚本。
...
**对话要求**：
- 每个角色发言5-7次，总共约{num_characters * 6}段对话（控制在20-30轮以内）
- 每段对话40-60字，包含具体观点、例子、解释或讨论
- 播客时长约3-4分钟
- 对话总字数建议1200-1800字（纯对话内容）
...
- **剧本格式**（SoulX-Podcast兼容格式）：
   ...
   - 每行一个角色的发言，角色之间建议有空行间隔，让对话更清晰
   - 角色名称必须使用：{role_list}
   - 每个角色发言3-5次即可，总共约{num_characters * 4}段对话（控制在15-20轮对话以内，以加快生成速度）
   - **角色间隔**：角色对话之间要有自然的间隔，每个角色发言后要有适当的停顿，让对话节奏更舒缓
