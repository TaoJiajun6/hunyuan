...
现在请开始生成，直接输出对话内容，不要添加任何其他说明、注释或解释。"""
    ...
    def build_streaming_podcast_prompt(
        self,
        text_material: str,
        instruction: Optional[str] = None
    ) -> str:
        """
        构建流式播客脚本生成提示词（单人播客，适配流式播放）
        
        Args:
            text_material: 输入内容（报告、文章、访谈稿、数据手册、故事素材等）
            instruction: 用户指令（可选），用于控制生成过程
...
现在请开始生成播客脚本，直接输出脚本内容，不要添加任何其他说明、注释或解释。"""
        return prompt
