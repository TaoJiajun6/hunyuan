...
- **⚠️ 时长控制（最重要，必须严格按照用户指令执行）**：
  ...
  - 如果用户指令中指定了"5分钟"或更长：每个角色发言12-15次，总共约{num_characters * 14}段对话，对话总字数约3000-4500字
  - 如果用户指令中指定了"深度播客"或"深度内容"：每个角色发言15-18次，总共约{num_characters * 17}段对话，对话总字数约4000-6000字，确保播客时长至少5-8分钟
  - 如果没有指定时长，默认使用5-8分钟的标准：每个角色发言12-15次，总共约{num_characters * 14}段对话，对话总字数约3000-4500字
    ...
    def build_deep_podcast_prompt(
        ...
        depth_requirements = {
            "深度": {
                ...
                "duration": "5-8分钟（至少5分钟）",
                "dialogue_count": "每个角色发言15-18次，总共约{num_characters * 17}段对话（控制在60-80轮对话以内）",
                "content_length": "对话总字数建议4000-6000字（纯对话内容，确保内容充实，至少达到5分钟时长）"
            },
            "中等": {
                ...
                "duration": "5-7分钟",
                "dialogue_count": "每个角色发言12-15次，总共约{num_characters * 14}段对话（控制在50-60轮对话以内）",
                "content_length": "对话总字数建议3000-4500字（纯对话内容，确保内容充实）"
            },
            "浅层": {
                ...
                "ending": "可以有明确的结论或总结",
                "duration": "3-5分钟",
                "dialogue_count": "每个角色发言8-10次，总共约{num_characters * 9}段对话（控制在30-40轮对话以内）",
