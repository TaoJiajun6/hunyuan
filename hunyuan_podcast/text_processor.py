...
**重要：本次播客要求时长达到4-5分钟，请生成足够丰富和深入的对话内容。对话总字数建议1500-2500字（纯对话内容）。**

**性能优化要求：为了提升生成速度，请生成较长的单段对话（每段50-80字），减少对话段数（每个角色发言10-12次即可），这样可以在保持总字数和时长的同时，显著减少生成时间。**
...
- **内容充实**：避免简短的回答，每个角色的发言都要有实质性内容（建议50-80字/段），能够推动讨论深入
- **性能优化**：生成较长的单段对话（50-80字），减少总段数，可以在保持总字数的同时显著减少生成时间
...
- 对话段数：约{num_characters * 11}段（每个角色10-12次发言，优化：减少段数以提升生成速度）
- 每段对话：每句话至少50-80字（优化：增加单段长度），包含具体观点、例子、解释或讨论
- 内容深度：要充分展开讨论，包含多个子话题、深入分析、案例分享、观点碰撞等
- **性能优化**：通过生成更长的单段对话来减少总段数，可以在保持总字数和时长的同时，显著减少生成时间
