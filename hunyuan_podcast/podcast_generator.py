    ...
    def generate_from_text_streaming(
        ...
        Yields:
            Tuple[int, torch.Tensor, bool, str]: (segment_index, audio_tensor, is_silence, role_name)
                - segment_index: 片段索引
                - audio_tensor: 音频张量 (1, samples)，采样率24000Hz
                - is_silence: 是否为静音片段
        ...
        # 使用流式TTS生成音频
        for segment_index, audio_tensor, is_silence, role_name in self.tts.infer_multi_speaker_streaming(
            speakers=speakers,
            dialogues=dialogues,
            silence_interval=silence_interval,
            verbose=verbose
        ):
            yield (segment_index, audio_tensor, is_silence, role_name)
