    ...
    with gr.Blocks(title="混元AI播客生成系统", theme=gr.themes.Soft(), css=custom_css) as demo:
        ...
        with gr.Tab("多角色互动播客", id="multi_role"):
            with gr.Accordion("📖 使用说明", open=False):
                ...
                **功能说明：**
                ...
                - 直接输入文本（支持角色标记或普通文本）
                - 如果输入普通文本，系统会自动调用混元大模型将其转换为多角色对话
                
                **文本格式要求：**
                - 使用 `[角色名]` 格式标记每个角色的发言
                - 角色名可以是任意名称，如 `[角色A]`、`[主持人]`、`[嘉宾]` 等
                - 支持多行文本，每行可以包含多个角色的对话
                - 支持情绪标注，如 `[角色A]（兴奋地）：太好了！`
                ...
                **使用步骤：**
                ...
                2. （可选）点击"🤖 自动分析文本"按钮，系统会自动分析文本并填充相关信息
                3. 为每个角色上传或录制音色参考音频（至少2个，按角色顺序：第一个角色→角色A音色，第二个角色→角色B音色）
                   - 可以上传音频文件（WAV、MP3等格式）
                   - 也可以点击麦克风图标直接录制（需要浏览器授权麦克风权限）
                4. （可选）在"高级设置"中配置播客名称、主题、角色设定、场景类型等
                5. （可选）在"背景音乐设置"中配置背景音乐（默认启用AI自动选择）
                6. 点击"🚀 生成播客"按钮
                7. 等待生成完成（可能需要几分钟）
                8. 播放生成的播客音频，查看生成的脚本
                ...
                **注意事项：**
                ...
                - 系统会自动选择背景音乐，无需手动指定（可在"背景音乐设置"中关闭AI自动选择并手动上传）
                - 生成过程可能需要几分钟，请耐心等待
                """)
            ...
            with gr.Row():
                with gr.Column(scale=1):
                    ...
                    silence_interval = gr.Slider(
                        ...
                        value=300,
                        step=50,
                        info="调整角色之间的静音间隔，建议范围：200-400毫秒。较小的间隔可以让对话更流畅自然。同一角色连续发言时会自动使用更短的间隔。"
                    )
        ...
        with gr.Tab("自定义角色播客", id="character"):
            with gr.Accordion("📖 使用说明", open=False):
                ...
                **使用说明：**
                1. 为每个角色设置以下信息（至少需要2个角色）：
                   - **角色名称**：为角色设置一个易于识别的名称
                   - **身份/职业**：描述角色的职业或身份
                   - **核心性格**：描述角色的性格特点
                   - **口头禅/说话习惯**：描述角色的说话习惯和口头禅
                   - **说话风格**：描述角色的声音特点和说话风格
                   - **与其他角色的关系**：描述角色与其他角色的关系
                   - **音色文件**：上传或录制角色的音色参考音频
                2. 输入播客主题（可选，如果留空，系统将根据角色人设自由生成对话）
                3. 点击"🚀 生成播客"按钮
                4. 等待生成完成（可能需要几分钟）
                5. 播放生成的播客音频，查看生成的脚本
                ...
                **角色A（托尼老师）：**
                - 身份/职业：时尚潮人、理发店总监
                - 核心性格：自信略带浮夸、热心肠
                - 口头禅/说话习惯：喜欢用夸张的赞美和比喻，语速快，充满激情，常夹杂英文单词和网络流行语
                - 说话风格：清亮有穿透力的声音，语调起伏大，常带笑意
                - 与其他角色的关系：与角色B是好友，经常互怼
                
                **角色B（程序员阿哲）：**
                - 身份/职业：资深后端工程师
                - 核心性格：逻辑控、内向务实、轻微社恐
                - 口头禅/说话习惯：语速平缓，用词精准，喜欢用'从技术实现上讲...'、'这个需求不明确'等句式
                - 说话风格：低沉温和的声音，语调平稳，偶尔有思考时的停顿
                - 与其他角色的关系：与角色A是好友，经常被角色A的热情感染
        ...
        with gr.Tab("主题深度播客", id="deep"):
            with gr.Accordion("📖 使用说明", open=False):
                ...
                **使用说明：**
                ...
                2. 选择角色数量（2-3个）
                3. 选择深度级别：
                   - **深度**：生成有深度、有见地的内容，引用理论、数据、案例等
                   - **中等**：生成中等深度的内容，平衡深度和可理解性
                   - **浅层**：生成浅层内容，更易于理解
                4. 为每个角色上传或录制音色文件（点击麦克风图标可录制）
                   - 角色A音色：第一个角色的音色
                   - 角色B音色：第二个角色的音色
                   - 角色C音色（可选）：第三个角色的音色（如果选择3个角色）
                5. 点击"🚀 生成播客"按钮
                6. 等待生成完成（可能需要几分钟）
                7. 播放生成的播客音频，查看生成的脚本
                ...
                **深度级别说明：**
                - **深度**：适合需要深入思考的主题，会引用理论、数据、案例等，使用开放式结尾
                - **中等**：适合一般性讨论，平衡深度和可理解性
                - **浅层**：适合入门性主题，更易于理解，适合普通听众
