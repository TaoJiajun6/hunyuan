...
class IndexTTS2:
    ...
    def _load_and_cut_audio(self,audio_path,max_audio_length_seconds,verbose=False,sr=None):
        ...
        优先使用 torchaudio.load（支持更多格式），如果失败则回退到 librosa.load
        """
        audio = None
        loaded_sr = None
        
        # 首先尝试使用 torchaudio.load（支持更多格式，如 MP3, M4A 等）
        try:
            if verbose:
                print(f"尝试使用 torchaudio 加载音频: {audio_path}")
            audio_torch, loaded_sr = torchaudio.load(audio_path)
            
            # torchaudio 返回的形状是 [channels, samples]，需要转换为 numpy 数组
            # 如果是多声道，转换为单声道
            if audio_torch.shape[0] > 1:
                audio_torch = torch.mean(audio_torch, dim=0, keepdim=True)
            
            # 转换为 numpy 数组（librosa 格式）
            audio = audio_torch.squeeze(0).numpy()
            
            # 如果指定了采样率，进行重采样
            if sr and loaded_sr != sr:
                if verbose:
                    print(f"重采样音频: {loaded_sr} Hz -> {sr} Hz")
            ...
            if verbose:
                print(f"使用 torchaudio 成功加载音频: 采样率={loaded_sr} Hz, 长度={len(audio)} 样本")
                
        except Exception as e:
            if verbose:
                print(f"torchaudio 加载失败: {str(e)}，尝试使用 librosa...")
            
            # 回退到 librosa.load
            try:
                if not sr:
                    audio, loaded_sr = librosa.load(audio_path)
                    sr = loaded_sr
                else:
                ...
                if verbose:
                    print(f"使用 librosa 成功加载音频: 采样率={loaded_sr} Hz, 长度={len(audio)} 样本")
                    
            except Exception as e2:
                error_msg = (
                    f"无法加载音频文件 '{audio_path}':\n"
                    f"  - torchaudio 错误: {str(e)}\n"
                    f"  - librosa 错误: {str(e2)}\n"
                    f"请检查文件格式是否正确，或尝试使用 ffmpeg 转换格式"
                )
                if verbose:
                    print(error_msg)
                raise RuntimeError(error_msg) from e2
        
        # 转换为 torch tensor，形状为 [1, samples]
        audio = torch.tensor(audio).unsqueeze(0)
        max_audio_samples = int(max_audio_length_seconds * loaded_sr)
    ...
    def remove_action_descriptions(self, text):
        """
        移除文本中的动作描述词，如 (点头)、(思考状) 等括号内的内容
        这些内容不应该被读入音频中
        """
        # 匹配中文括号和英文括号内的内容
        # 移除类似 (点头)、(思考状)、(Nods)、(Thinking) 等动作描述
        # 使用正则表达式匹配括号及其内容，支持中文括号（）和英文括号()
        # 使用非贪婪匹配，避免匹配跨行的内容
        text = re.sub(r'[（(][^）)]*?[）)]', '', text)
        # 清理可能出现的多余空格
    ...
    def infer_generator(self, spk_audio_prompt, text, output_path,
        ...
        # 移除动作描述词（如：点头、思考状等括号内的内容）
        text = self.remove_action_descriptions(text)
        if verbose:
            print(f"text after removing action descriptions: {text}")
